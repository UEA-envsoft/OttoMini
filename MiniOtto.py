...
class Otto:
    ...
    # --  Otto initialization
    def init(self, LL, RL, LF, RF, load_calibration, Buzzer,U_trg,U_ech):
        self._servo_pins[0] = LL
        self._servo_pins[1] = RL
        self._servo_pins[2] = LF
        self._servo_pins[3] = RF
        ...
        self.trg_pin = Pin(U_trg, Pin.OUT)
        self.ech_pin = Pin(U_ech, Pin.IN)
    ...
    # -- Oscillator trims
    def setTrims(self, LL, RL, LF, RF):
        self._servo[0].SetTrim(0 if LL is None else LL)
        self._servo[1].SetTrim(0 if RL is None else RL)
        self._servo[2].SetTrim(0 if LF is None else LF)
        self._servo[3].SetTrim(0 if RF is None else RF)
